...
export const adminOperationProtocol = "holm.sdk.admin-operation/1" as const;
...
export type AdminDBRetentionOperationOptions = Pick<
  AdminOperationOptions<Record<never, AdminPathValue>>,
  "headers" | "timeoutMs" | "sensitive" | "control" | "reason"
>;

export type AdminOperationInput<
  Path extends AdminPathValues,
  RequiresPath extends boolean,
> = RequiresPath extends true
  ? Omit<AdminOperationOptions<Path>, "path"> & { readonly path: Path }
  : AdminOperationOptions<Path>;

export type AdminMethodArguments<
  Path extends AdminPathValues,
  RequiresPath extends boolean,
  RequiresInput extends boolean,
> = RequiresInput extends true
  ? readonly [input: AdminOperationInput<Path, RequiresPath>]
  : readonly [input?: AdminOperationInput<Path, RequiresPath>];

export type AdminRouteMethod<
  Name extends string,
  Path extends AdminPathValues = Record<never, AdminPathValue>,
  RequiresPath extends boolean = false,
  RequiresInput extends boolean = RequiresPath,
> = Name extends string
  ? <Result = WireValue>(
      ...args: AdminMethodArguments<Path, RequiresPath, RequiresInput>
    ) => Promise<Result>
  : never;

export type AdminDBRetentionStatusMethod = (
  input?: AdminDBRetentionOperationOptions,
) => Promise<AdminDBRetentionStatus>;

export type AdminDBRetentionRunMethod = (
  input?: AdminDBRetentionOperationOptions,
) => Promise<AdminDBRetentionReport>;

export type AdminUrlHelper<
  Name extends string,
  Path extends AdminPathValues = Record<never, AdminPathValue>,
  RequiresPath extends boolean = false,
  RequiresInput extends boolean = RequiresPath,
> = Name extends string
  ? (...args: AdminMethodArguments<Path, RequiresPath, RequiresInput>) => string
  : never;
