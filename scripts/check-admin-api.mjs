...
const typedBodylessMethods = new Map([
  ["system.dbRetentionStatus", "AdminDBRetentionStatusMethod"],
  ["system.dbRetentionRun", "AdminDBRetentionRunMethod"],
]);
  ...
  for (const entry of document.entries) {
    entry.sdk.forEach((name, index) => {
      ...
      const descriptor = byName.get(name) ?? {
        ...
        routes: [],
        ...(typedBodylessMethods.has(name) ? { requestBody: "forbidden" } : {}),
      };
...
function validateDescriptors(document, descriptors, failures) {
  ...
  for (const name of typedBodylessMethods.keys()) {
    if (!names.includes(name)) failures.push(`required typed bodyless method ${name} is missing`);
  }
  ...
  for (const descriptor of descriptors) {
    ...
    if (typedBodylessMethods.has(descriptor.name) && descriptor.requestBody !== "forbidden") {
      failures.push(`${descriptor.name} must remain bodyless`);
    }
...
function renderGenerated(descriptors) {
  ...
  const descriptorText = descriptors.map((descriptor) => {
    ...
    const fields = [
      ...
      ...(descriptor.requestBody === undefined ? [] : [`requestBody: ${JSON.stringify(descriptor.requestBody)}`]),
    ];
...
function renderInterface(name, node, depth) {
  ...
  for (const key of Object.keys(node).sort()) {
    ...
    if (value.name !== undefined) {
      lines.push(`${childIndent}readonly ${safeProperty(key)}: ${renderMethodType(value)};`);
    } else {
...
function renderObjectMembers(node, depth) {
  ...
  for (const key of Object.keys(node).sort()) {
    const value = node[key];
    if (value.name !== undefined) {
      lines.push(`${indent}readonly ${safeProperty(key)}: ${renderMethodType(value)};`);
    } else {
...
function renderMethodType(value) {
  const typedMethod = typedBodylessMethods.get(value.name);
  if (typedMethod !== undefined) return typedMethod;
  ...
  return `${methodType}<${JSON.stringify(value.name)}, ${pathType}, ${everyRouteNeedsPath}, ${everyRouteNeedsPath || forceInput}>`;
}
