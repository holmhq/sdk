...
- [x] Strict red → green → refactor tests cover creation, teardown, capability
      negotiation, extension ordering/conflicts, caller propagation, and aborts.
      (`S04`-`S08`, final Issue `#004` review `92ec30a`)
- [x] Core compiles and runs with no DOM or Node ambient/runtime dependency.
      (`S08`: `37604c3`, fix `e180ba0`, re-review `92ec30a`)
- [x] Two independent instances can use different adapters/callers without
      shared state leakage. (`S08`: `37604c3`, fix `e180ba0`, re-review
      `92ec30a`)
...
- [x] Mock adapter and fake clock make behavior deterministic. (`S08`:
      `37604c3`, fix `e180ba0`, re-review `92ec30a`)
- [x] Generated ESM/declarations pass consumer tests. (`S08` validation:
      `npm run test:declarations`; `npm run test:dist`)
- [x] Public exports and bundle contribution are documented and size-reported.
      (`S08` validation: `npm run size`)
...
- `S08` completed Issue `#004` with `createHolm`, lifecycle/cancellation,
  deterministic fakes, instance isolation, test exports, and generated artifact
  proof.
