...
import { applyTransportAuth, createTransportCache, createTransportRequest, decodeTransportResponse, encodeTransportBody, normalizeTransportError, } from "../transports/index.js";
import { createNodeRuntimeServices, createUnsupportedClock, createUnsupportedScheduler, UnsupportedNodeRuntimeServiceError, } from "./services.js";
...
export function nodeRuntime(options = {}) {
    ...
    return Object.freeze({
        ...
        async invoke(request, control) {
            ...
            throwIfRuntimeDisposed(disposed);
            const source = Object.freeze({ id, surface: "cli" });
            ...
            const response = await waitForNodeResponse(executeFetch(authenticated, url, request.requestId, control.cancellation), control.cancellation);
            if (cache !== undefined) {
                cache.invalidateForMutation({ tags: [callerCacheTag(request.callerFingerprint)] });
            }
            return response;
...
function createNodeRuntimeCache(options, clock, scheduler, diagnostics) {
    if (options === false) {
        return undefined;
    }
...
function createNodeCachePolicy(options) {
    return Object.freeze({
...
async function authenticateNodeRequest(request, auth, adapterId, cancellation) {
    if (auth === undefined) {
...
function requireFetch(fetchImplementation, adapter) {
...
function assertHttpOperation(request, adapterId) {
    ...
    if (expected === HOLM_ADMIN_HTTP_CAPABILITY && request.caller.principal.kind !== "operator") {
        throw new UnsupportedCapabilityError({
            ...context,
            message: "The Node/CLI admin transport requires an explicit operator caller context; Holm still enforces authorization.",
        });
    }
...
function resolveNodeRequestUrl(value, baseUrl, params = {}) {
...
function copyBytesToUint8Array(bytes) {
...
function waitForNodeResponse(response, cancellation) {
...
function rebindResponseRequestId(response, requestId) {
    return Object.freeze({
        ...response,
        requestId,
    });
