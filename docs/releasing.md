...
branch protection appropriate to the organization. The publishing job names
this environment exactly, so npm can bind its OIDC trust to the same deployment
boundary.
...
6. Confirm the workflow checks out the peeled tag target, passes all gates, and
   reports a staged npm package.
7. On npmjs.com, open the package's **Staged Packages** tab. Review package name,
   version, files, integrity/provenance, and source workflow, then click
   **Approve** and complete WebAuthn.
8. Verify npm `latest`, registry shasum/integrity, and a clean registry install
   of every entry point.
9. Create or finalize the GitHub release with tarball, `SHA256SUMS`, and
   `dist-manifest.json`; download and compare every asset byte-for-byte.
10. Commit/push release state and close only with clean, synchronized Git.

The staging workflow does not create tags, GitHub releases, or package versions.
Those remain explicit reviewed release steps. It cannot approve its own staged
package.
